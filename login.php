...
$clavemd5=md5($clave);
$pasa=0;
$ssql="select * from usuarios where email='$email' and clave='$clavemd5'";
if($conx->getCantidadReg($ssql)>0)
    $pasa=1;
else {
    $ssql="select * from usuarios where email='$email'";
    if($conx->getCantidadReg($ssql)>0) {
        if($conx->Passgral()==$clavemd5) $pasa=1;
    }
}
...
if($pasa==1) {
    ...
    $_SESSION["certificadorem"]="";
    $_SESSION["paisrem"]=0;
    $_SESSION["articulosselrem"]="";
    ...
    $_SESSION["fechafininf"]=date("Y-m-d");
    $_SESSION["paisinf"]=0;
    $_SESSION["articulosselinf"]=0;
    
    //ingresos
    $_SESSION["limhor"]=0;    
    $_SESSION["iddephor"]=0;        
    $_SESSION["idemphor"]=0;        
    $_SESSION["fechainihor"]=date("Y-m-01");
    $_SESSION["fechafinhor"]=date("Y-m-d");
    
    // empleados
    $_SESSION["limemp"]=0; 
    $_SESSION["detalleemp"]=0;
    
    //adelantos
    $_SESSION["limade"]=0;
    $_SESSION["fechainiade"]=date("Y-m-01");
    $_SESSION["fechafinade"]=date("Y-m-d");
    $_SESSION["personalade"]="";  
        
    //extras
    $_SESSION["fechainiext"]=date("Y-m-01");
    $_SESSION["fechafinext"]=date("Y-m-d");
    $_SESSION["empleadoext"]=0;
    $_SESSION["limext"]=0;    
    
    //asistecias
    $_SESSION["anomesasi"]=date("Ym");
    $_SESSION["idempasi"]=0;
    
//    $_SESSION["fechainiasi"]=date("Y-m-01");
//    $_SESSION["fechafinasi"]=date("Y-m-d");
//    $_SESSION["limasi"]=0;   
//    $_SESSION["idperasi"]=0;   
