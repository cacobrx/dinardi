...
//                    echo $ssql."\n";
                    if($conx->getCantidadRegA($ssql, $conn)>0) {
                        ...
                        array_push($control, $rtt->id);
//                        echo "imp: $imp\n";
//                        echo "1- ".$rdd->cantidad." * ".$rtt->precio." -- ".$rtt->cantidad."\n";
//                        echo "id: ".$rdd->id."\n";
                    } else {
                        $imp+=$rdd->cantidad*$pre->getPreciofinal();
                        $ximp=$rdd->cantidad*$pre->getPreciofinal();
//                        echo "2- ".$rdd->cantidad." * ".$pre->getPreciofinal()."\n";
                    }
                } else {
                    $rf=$conx->consultaBase($ssql, $conn);
                    $rff=mysqli_fetch_object($rf);
                    $imp+=$rdd->cantidad * $rff->precio;
                    $ximp=$rdd->cantidad * $rff->precio;
//                    echo "3- ".$rdd->cantidad." * ".$rff->precio."\n";
                }
...
//        echo $ssql."<br>";
        ...
        while($reg=mysqli_fetch_object($rs)) {
            $fecha=$fechaini;
            $ssql="select adm_crm_det.*, adm_rem.idprv, adm_crm.idrem, adm_crm.fecha from adm_crm_det, adm_crm, adm_rem where adm_crm.fecha>='$fechaini' and adm_crm.fecha<='$fechafin' and adm_rem.idprv=".$reg->id." and adm_crm.id=adm_crm_det.idcrm and adm_crm.idrem=adm_rem.id";
            if($idart>0) $ssql.=" and adm_crm_det.idart=$idart";
            if($solofaena==1) $ssql.=" and adm_rem.faena=1";
            if($sinfaena==1) $ssql.=" and adm_rem.faena=0";
//            echo $ssql."\n";
...
//                        echo "id: ".$rdd->id."<br>";
                ...
                $neto+=$xneto;
                $iva+=$xiva;
                $search=array_search($reg->id."|".$rdd->fecha."|".$rdd->idart, $cad1);
                if($search===false) {
                    $art=new adm_art_1($rdd->idart, $conn);
                    array_push($cad1,$reg->id."|".$rdd->fecha."|".$rdd->idart);
                    array_push($xv_cantidad,$rdd->cantidad);
                    array_push($xv_fecha,$rdd->fecha);
                    array_push($xv_id,$reg->id);
                    array_push($xv_idart,$rdd->idart);
                    array_push($xv_iva,$xiva);
                    array_push($xv_neto,$xneto);
                    array_push($xv_total,$ximp);
                    array_push($xv_proveedor,$reg->apellido." ".$reg->nombre);
                    array_push($xv_art,$art->getDescripcion());
                } else {
                    $xv_iva[$search]+=$xiva;
                    $xv_neto[$search]+=$xneto;
                    $xv_total[$search]+=$ximp;
                }
            }
            
            $prv=new adm_prv_1($reg->id, $conn);
            $cadx=$prv->getApellido()." ".$prv->getNombre()."|$fecha|$can|$imp|$neto|$iva";
            switch ($orden) {
                case 1: // x cliente
                    $or=$prv->getApellido()." ".$prv->getNombre();
                    break;
...
//            echo "cadx: $cadx\n";
        ...
        for($i=0;$i<count($cad1);$i++) {
            array_push($cad,$xv_fecha[$i]."|".$xv_proveedor[$i]."|".$xv_art[$i]."|".$xv_idart[$i]."|".$xv_cantidad[$i]."|".$xv_neto[$i]."|".$xv_iva[$i]."|".$xv_total[$i]);
        }
        sort($cad);
        for($i=0;$i<count($cad);$i++) {
            $xcad=explode("|",$cad[$i]);
            array_push($this->proveedor,$xcad[1]);
            array_push($this->idart,$xcad[3]);
            array_push($this->cantidad,$xcad[4]);
            array_push($this->fecha,$xcad[0]);
            array_push($this->importe,$xcad[7]);
            array_push($this->iva,$xcad[6]);
            array_push($this->neto,$xcad[5]);
            array_push($this->articulo,$xcad[2]);
        }
...
//            if($xcad[3]>0) {
//                array_push($this->articulos,$xcad[1]);
//                array_push($this->cantidad,$xcad[3]);
//                array_push($this->importe,$xcad[4]);
//                array_push($this->neto,$xcad[5]);
//                array_push($this->iva,$xcad[6]);
//                array_push($this->fecha,$xcad[2]);
//            }
//        }
    }
