...
//        echo "fechaini: $fechaini | fechafin: $fechafin<br>\n";
        ...
        // ingresos
        if($civa==1)
            $ssql="select sum(netori21+netori10+netocf21+netocf10+ivacf21+ivacf10+ivari21+ivari10+percepcioniibb+nogravado) as totaling from adm_fis where fecha>='$fechaini' and fecha<='$fechafin' and (tipo='F' or tipo='G' or tipo='D' or tipo='I')";
        else 
            $ssql="select sum(netori21+netori10+netocf21+netocf10+nogravado) as totaling from adm_fis where fecha>='$fechaini' and fecha<='$fechafin' and (tipo='F' or tipo='G' or tipo='D' or tipo='I')";
...
//        $ssql.=" where adm_crem.fecha>='$fechaini' and adm_crem.fecha<='$fechafin'";
        ...
        if($civa==1)
            $ssql="select sum(netori21+netori10+netocf21+netocf10+ivacf21+ivacf10+ivari21+ivari10+percepcioniibb+nogravado) as totaling from adm_fis where fecha>='$fechaini' and fecha<='$fechafin' and (tipo='C' or tipo='H')";
        else 
            $ssql="select sum(netori21+netori10+netocf21+netocf10+nogravado) as totaling from adm_fis where fecha>='$fechaini' and fecha<='$fechafin' and (tipo='C' or tipo='H')";
//        $ssql="select sum(precio*cantidad) as totaling from adm_crem_det inner join adm_crem on adm_crem_det.idrem=adm_crem.id";
//        $ssql.=" where adm_crem.fecha>='$fechaini' and adm_crem.fecha<='$fechafin'";
        //echo $ssql."\n";
        $ri=$conx->consultaBase($ssql, $conn);
        ...
        if($civa==1)
            $ssql="select sum(neto21+neto10+iva21+iva10+perretiibb+periva+retiva+nogravado+exento) as totcom1 from adm_com where fecha>='$fechaini' and fecha<='$fechafin' and tipocom!=2 and tipo=1";
        else
            $ssql="select sum(neto21+neto10+nogravado+exento) as totcom1 from adm_com where fecha>='$fechaini' and fecha<='$fechafin' and tipocom!=2 and tipo=1";
...
//        echo $ssql."\n";
        ...
        //echo $rcc->totcom1."\n";
        if($civa==1)
            $ssql="select sum(neto21+neto10+iva21+iva10+perretiibb+periva+retiva+nogravado+exento) as totcom2 from adm_com where fecha>='$fechaini' and fecha<='$fechafin' and tipocom=2 and tipo=1";
        else
            $ssql="select sum(neto21+neto10+nogravado+exento) as totcom2 from adm_com where fecha>='$fechaini' and fecha<='$fechafin' and tipocom=2 and tipo=1";
...
//        echo $ssql."\n";
        ...
        if($civa==1) {
            $percepciones=0;
            $ssql="select sum(retiva + periva + perretiibb + impinternos) as totalimp from adm_com inner join adm_prv on adm_com.idprv=adm_prv.id where fecha>='$fechaini' and fecha<='$fechafin' and adm_com.tipocom!=2";
//            echo $ssql."<br>";
            $ri=$conx->consultaBase($ssql, $conn);
            $rii=mysqli_fetch_object($ri);
...
//            echo "ceros: $ceros<br>";
            ...
            $condiciongas=$this->getSqlcuenta0($c_cta[$i], 1, 0, "adm_com_det",$conn);
...
//            $ssqls="select sum(adm_com_det.importe) as totalsalida from adm_com_det, adm_com where adm_com_det.idcom=adm_com.id and adm_com.tipo=2 and ($condicionctadet) and adm_com.fecha>='$fechaini' and adm_com.fecha<='$fechafin' and tipocom!=2";
            $ssqlg="select sum(adm_com_det.importe) as totalgastos from adm_com_det inner join adm_gas on adm_gas.id=adm_com_det.idgas where adm_gas.fecha>='$fechaini' and adm_gas.fecha<='$fechafin' and ($condiciongas)";
//            echo $ssqle."\n";
            if($c_id[$i]==834)
            echo $ssqlg."<br>\n";
...
//                            echo $reg->importe." ".$reg->neto21." ".$xx."\n";
                        ...
                        if($reg->neto10>0) {
                            $xx=$reg->importe+$reg->importe*10.5/100;
                            $tots+=$xx;
                        }
                        if($reg->neto27>0) {
                            $xx=$reg->importe+$reg->importe*27/100;
                            $tots+=$xx;
                        }
                        if($reg->neto21==0 and $reg->neto10==0 and $reg->neto27==0) $tots+=$reg->importe;
                    } else
                        $tots+=$reg->importe;
                }
            }
