...
class adm_cli_pre_1 {
    ...
    function __construct($idcli, $idart, $conn="0") {
        require_once "clases/conexion.php";
        $conx=new conexion();
        if($conn=="0") $conn=$conx->conectarBase();
        $ssql="select * from adm_cli_pre where idcli=$idcli and idart=$idart";
...
//                echo "iiccllii: ".$reg->idart."<br>";
                $art=new adm_prd_1($reg->idart, $conn);
                array_push($this->descripcion,$art->getDescripcion());
                array_push($this->codigo,$art->getCodigoproducto());
