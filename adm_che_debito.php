...
for($i=0;$i<$cantidadact;$i++) {
    ...
    if($$chek>0) {
        $ccc=new adm_che_1($$chek, $conn);
        $ssql="update adm_che set fechadeb='".$ccc->getFechapago()."' where id=".$$chek;
        //echo $ssql."<br>";
        $conx->consultaBase($ssql, $conn);
        $aud->regAudC("Cheques Propios", $usr->getId(),"Cancela cheque #".$$chek." | Cheque: ".$ccc->getNrocheque(),$centrosel,$conn);
    }
}
?>
<html>
