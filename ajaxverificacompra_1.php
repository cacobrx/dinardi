...
if($conx->getCantidadReg($ssql)>0) {
    ...
    if($pto==0 or $nro==0) {
        ...
        if($val!="") {
            $ssql="select * from adm_com where idprv=$val and ptovta=$pto and numero=$nro and tipocom=$tip";
            $rep=$conx->getCantidadReg($ssql);
            echo "rep1: ".$ssql."<br>";
            $ssql="select * from adm_com where ptovta=$pto and numero=$nro and idprv!=$val and tipocom=$tip";
            echo "ret1: ".$ssql."<br>";
            $ret=$conx->getCantidadReg($ssql);
        } else {
            $rep=0;
            $ret=0;
            $ssql="select * from adm_com where ptovta=$pto and numero=$nro and tipocom=$tip";
            $ret=$conx->getCantidadReg($ssql);
            echo "ret2: ".$ssql."<br>";
        }
    }
