...
$p_idprv=$adm->getPrv_idprv();
//print_r($p_idprv);

$ssql="select * from adm_art where envasado=1 order by descripcion";    
$art=new adm_art_2($ssql);
$a_id=$art->getId();
$a_des=$art->getDescripcion();
...
<div id="barcentral">
<form name="form1" id="form1" action="adm_ela_mod_save.php" method="post">
    <tr>
                                            ...
                                            <div class="panelmax letra6">
                                                <div id="effect-panelmax910" class="ui-widget-content ui-corner-all">
                                                    <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="letra6" id="item_table">
                                                        ...
                                                        <tr>
                                                            <td><input type="date" id="item_fecha<?= $i?>" name="item_fecha<?= $i?>" value="<?=$d_fec[$i]?>" class="letra6" /></td>
                                                            <td><select name="item_producto<?= $i?>" id="item_producto<?= $i?>">
                                                                   <?

                                                                   $sup->cargaComboArrayValor($a_des, $a_id,$d_art[$i]);
                                                                   ?>
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <select name="item_proveedor<?= $i?>_0" id="item_proveedor<?= $i?>_0">
                                                                   <?
                                                                   $sup->cargaComboArrayValor($p_ape, $p_id,$p_idprv[$i][0],"Seleccionar");
                                                                   ?>
                                                                </select>
                                                                <select name="item_proveedor<?= $i?>_1" id="item_proveedor<?= $i?>_1">
                                                                   <?
                                                                   $sup->cargaComboArrayValor($p_ape, $p_id,$p_idprv[$i][1],"Seleccionar");
                                                                   ?>
                                                                </select>
                                                                <select name="item_proveedor<?= $i?>_2" id="item_proveedor<?= $i?>_2">
                                                                   <?
                                                                   $sup->cargaComboArrayValor($p_ape, $p_id,$p_idprv[$i][2],"Seleccionar");
                                                                   ?>
                                                                </select>
                                                            </td>                                                            
                                                            <td><input type="text" id="item_kgdescarte<?= $i?>" name="item_kgdescarte<?= $i?>" value="<?= $d_kgd[$i]?>" size="10" maxlength="10" onkeypress="return validar_punto(event)" style="text-align: center" /></td>
                                                            <td><input type="text" id="item_kgfinal<?= $i?>" name="item_kgfinal<?= $i?>" value="<?= $d_kgf[$i]?>" size="10" maxlength="10" onkeypress="return validar_punto(event)" style="text-align: center" /></td>
...
$(document).ready(function(){
 ...
 $(document).on('click', '.add', function(){
  ...
  html += '<tr>';
  html += '<td><input type="date" id="item_fecha' + cantidaddet + '" name="item_fecha' + cantidaddet + '" class=\"letra6\" value=\"<?= date("Y-m-d")?>\" /></td>';
