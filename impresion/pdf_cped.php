...
class PDF_cped extends pdf_function {
    // private variables
    var $colonnes;
    var $format;
    var $angle=0;
...
//        $this->Titulo("LISTADO DE PRODUCTOS", 0);
        $this->TituloRec("Pedidos de Clientes");
        $r1=5;
        $r2=205;
        $y1=35;
        $y2=290;
    ...
    function Detalle() {
        ...
        for($i=0;$i<count($a_id);$i++) {
            ...
            $this->Cell(20,5,"Total",0,1,"R");   
            $this->SetFont("Arial","",8);
            $ssql="select * from adm_cped_det where idped=".$a_id[$i]; 
//            echo $ssql;
            $adm=new adm_cped_det_2($ssql);
            $d_can=$adm->getCantidad();
            $d_art=$adm->getArticulo();
            $d_rec=$adm->getRecipiente();
            $d_pre=$adm->getPrecio();
            $d_imp=$adm->getImporte();            
            for($d=0;$d<count($d_can);$d++) {
                $this->SetX($colu2[0]);
                $this->Cell(10,5,$d_can[$d],0,0,"C");
                $this->SetX($colu2[1]);
                $this->Cell(10,5,$d_art[$d],0,0,"L");
                $this->SetX($colu2[2]);
                $this->Cell(10,5,$d_rec[$d],0,0,"C");                
                $this->SetX($colu2[3]);
