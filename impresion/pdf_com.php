...
class PDF_Com extends pdf_function {
    // private variables
    var $colonnes;
    var $format;
    var $angle=0;
    ...
    function Header() {
        ...
        $this->fact_dev( utf8_decode("Compras"),0,125);
...
//        $this->TituloRec("PRODUCTOS");
    ...
    function addDetalle() {
        ...
        for($i=0;$i<count($a_id);$i++) {
            ...
            $this->SetX($colu[4]);
            $this->Cell(20,5, number_format($a_neto21[$i]+$a_neto10[$i]+$a_neto27[$i]+$a_exe[$i]+$a_ngr[$i],2),0,0,"R");
            $this->SetX($colu[5]);
            $this->Cell(20,5, number_format($a_iva21[$i] + $a_iva10[$i] + $a_iva27[$i] + $a_imi[$i],2),0,0,"R");
            $this->SetX($colu[6]);
            $this->Cell(20,5, number_format($a_pri[$i] + $a_rti[$i] + $a_prb[$i],2),0,0,"R");
            $this->SetX($colu[7]);
            $this->Cell(20,5, number_format($a_neto21[$i] + $a_neto10[$i] + $a_neto27[$i] + $a_iva21[$i] + $a_iva10[$i] + $a_iva27[$i] + $a_imi[$i] + $a_pri[$i] + $a_rti[$i] + $a_prb[$i] + $a_exe[$i] + $a_ngr[$i],2),0,0,"R");
            $this->SetX($colu[3]);
        ...
        $this->setX($colu[7]);
        $this->Cell(20,5, number_format(array_sum($a_neto21) + array_sum($a_neto10) + array_sum($a_neto27) + array_sum($a_iva21) + array_sum($a_iva10) + array_sum($a_iva27) + array_sum($a_imi) + array_sum($a_pri) + array_sum($a_rti) + array_sum($a_prb) + array_sum($a_exe) + array_sum($a_ngr),2),0,1,"R"); 
