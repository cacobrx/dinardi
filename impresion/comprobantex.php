...
//            $this->Cell(10,5,"",0,1);
            ...
            for($i=0;$i<count($d_art);$i++) {
                ...
                if($fis->getLetra()=="A") {
                    $this->SetX(155);
                    $this->Cell(20,5,number_format($precio*$d_can[$i],2),0,0,"C");
                    $this->SetX(175);
                    $this->Cell(10,5,number_format($d_iva[$i],2),0,0,"C");
                    $this->SetX(185);    
                    $this->Cell(20,5,number_format($precio*$d_can[$i]*$coef,2),0,1,"C");
                    switch ($d_iva[$i]) {
                        case 21:
                            $iva21+=$d_pre[$i]*$d_can[$i];
                            break;
                        case 10.5:
                            $iva10+=$d_pre[$i]*$d_can[$i];
                            break;
                    }
                } else {
                    $this->SetX(155);
                    ...
                    $this->SetX(175);
                    $this->Cell(30,5,number_format($d_pre[$i]*$d_can[$i],2),0,1,"C");
...
//            $this->Cell(100,5,  utf8_decode($fis->getTexto()),0,0);
            ...
            $cad=$cli->getCuit().$codigocomp.$sup->AddZeros($fis->getPtovta(), 4).$fis->getNumerocae().date("Ymd",strtotime($fis->getFechacae()));
