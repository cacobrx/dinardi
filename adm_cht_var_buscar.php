<?php
/*
 * Creado el 22/01/2020 17:21:43
 * Autor: gus
 * Archivo: adm_cht_var_buscar.php
...
//********************************
if($url=="") $url="adm_opg_var_act.php";

$ssql="select * from adm_che where acreditado=0 order by fechapago";
...
//$a_via=$cht->getTipo();
$cantidadche=$che->getMaxRegistros();
...
$ssql="select * from adm_cht where (entregado='' or entregado is null) and tipo=$tipo order by fechapago";
...
$a_ent=$cht->getEntregado();
$a_via=$cht->getTipo();
$cantidadcht=$cht->getMaxRegistros();
...
$apagar=$importe-$retenciones-$transferencia-$importeganancia-$totalseleccionadoche;
...
<script language="javascript">
    function totalsuma(can) {
        ...
        sal=value=parseFloat(document.getElementById("importe").value) -  tot - parseFloat(document.getElementById("retenciones").value) - parseFloat(document.getElementById("totalpagosche").value) - parseFloat(document.getElementById("importeganancia").value);
...
<form name="form1" id="form1" action="<?= $url?>" method="post">
    <tr>
        ...
        <input name="cantidadcht" id="cantidadcht" type="hidden" value="<?= count($a_id)?>" />
        ...
        <input name="totalpagosche" type="hidden" id="totalpagosche" value="<?= $totalpagosche?>" />
        <input name="cantidadtotal" type="hidden" id="cantidadtotal" value="<?= $cantidadtotal?>" />
                    ...
                    <td>
                        <div class="panelmax letra6">
                            <div id="effect-panelmax" class="ui-widget-content ui-corner-all">
                                <h3 class="ui-widget-header ui-corner-all">Selección de CHEQUES DE TERCEROS para el pago de O/PAGO</h3>              
                                <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
                                    <tr>
                                        <td>
                                            <a href="javascript: document.form1.action='adm_opg_var_act.php'; document.form1.submit()"><i class="fas fa-arrow-left fa-lg" title="Volver" alt="Volver"></i></a>
                                            Importe a Pagar <span class="letra6bold"><?= number_format($importe-$retenciones-$transferencia-$importeganancia,2)?></span> | 
                                            Total Seleccionado <input name="totaltotal" id="totaltotal" size="10" style="text-align: center" class="letra6bold" type="text" readonly="readonly" value="<?= $totalseleccionadocht?>" /> | 
                                            Saldo <input name="saldototal" id="saldototal" size="10" style="text-align: center" class="letra6bold" type="text" value="<?= $importe-$retenciones-$totalseleccionadocht-$transferencia-$importeganancia-$totalpagosche?>" /> | 
                                            <input name="cmdok" id="cmdok" value="FINALIZAR" type="submit" />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="ui-widget-header ui-corner-all"><span class="letra6">Cantidad: <?= $cantidadcht?></td>                      
                                    </tr>            
                                    <tr>
                                        <td>
...
//                                                    echo "chk1: $chk1: ".$$chk1."<br>";
                                                ...
                                                <tr onMouseOver="overTR(this,'<?= $cfg->getColorbarra()?>')" onMouseOut="overTR(this,'#ffffff')" class="letra3">
                                                    <td width="25%">
                                                        <input name="chkcht<?= $i?>" id="chkcht<?= $i?>" type="checkbox" value="<?= $a_id[$i]?>" onclick="javascript: totalsuma(<?= count($a_id)?>)" <? if($$chk0>0) echo "checked='checked'";?> /> 
                                                    ...
                                                    <? if($i+1<count($a_id)) { ?>
                                                    <td width="25%">
                                                        <input name="chkcht<?= $i+1?>" id="chkcht<?= $i+1?>" type="checkbox" value="<?= $a_id[$i+1]?>" onclick="javascript: totalsuma(<?= count($a_id)?>)" <? if($$chk1>0) echo "checked='checked'";?> /> 
                                                    ...
                                                    if($i+2<count($a_id)) { ?>
                                                    <td width="25%">
                                                        <input name="chkcht<?= $i+2?>" id="chkcht<?= $i+2?>" type="checkbox" value="<?= $a_id[$i+2]?>" onclick="javascript: totalsuma(<?= count($a_id)?>)" <? if($$chk2>0) echo "checked='checked'";?> /> 
                                                    ...
                                                    if($i+3<count($a_id)) { ?>
                                                    <td width="25%">
                                                        <input name="chkcht<?= $i+3?>" id="chkcht<?= $i+3?>" type="checkbox" value="<?= $a_id[$i+3]?>" onclick="javascript: totalsuma(<?= count($a_id)?>)" <? if($$chk3>0) echo "checked='checked'";?> /> 
